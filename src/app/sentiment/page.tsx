      ...
      <ResponsiveContainer width="100%" height={300}>
        <PieChart>
          ...
          <text x="50%" y="45%" textAnchor="middle" dominantBaseline="middle">
            <tspan x="50%" dy="-0.35em" fontSize={22} fontWeight={700} fill={cc.ts}>{total.toLocaleString()}</tspan>
            <tspan x="50%" dy="1.4em" fontSize={11} fill={cc.tm}>Total</tspan>
          </text>
          <Tooltip
            formatter={(value: any, name: any, props: any) => [
              `${props.payload.count.toLocaleString()} berita (${value}%)`, name
            ] as [string, string]}
            contentStyle={{ backgroundColor: "var(--bg-card)", border: `1px solid ${cc.bd}`, borderRadius: 12, fontSize: 11 }}
          />
          <Legend formatter={(value) => <span style={{ color: cc.ts, fontSize: 12 }}>{value}</span>} />
        </PieChart>
      </ResponsiveContainer>
    )}</ViewportChart>
  );
